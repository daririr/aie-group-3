"""Тесты для моделей и предсказаний."""
import pytest
...
def artifacts_exist():
    ...
    from pathlib import Path
    from src.config import ARTIFACTS_DIR  # ✅ Используем тот же путь, что и основной код
...
def test_predictor_risk_calculation(sample_input):
    ...
    # Тест с низким риском (малое значение X7 - glazing area)
    low_risk_input = sample_input.copy()
    low_risk_input["X7"] = 0.0  # минимальное остекление
    
    result = predictor.predict(low_risk_input)
    assert result["risk_level"] in ["low", "medium"]  # не должен быть high
