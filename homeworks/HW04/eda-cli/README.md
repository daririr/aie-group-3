...
## 5. `POST /quality-flags-from-csv` – флаги качества по CSV (HW04)

Эндпоинт, реализованный в рамках домашнего задания HW04.
Принимает **CSV-файл** и возвращает только словарь флагов качества, включая кастомные эвристики из HW03:

- `has_constant_columns` — есть ли колонки с одинаковыми значениями;
- `has_suspicious_id_duplicates` — есть ли дубликаты в user_id.
...
- `has_suspicious_id_duplicates` — есть ли дубликаты в колонке user_id;
- `too_few_rows` — меньше ли 100 строк;
- `too_many_columns` — больше ли 100 колонок;
- `too_many_missing` — превышает ли доля пропусков 50%.
