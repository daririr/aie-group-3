...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    flags_all = compute_quality_flags(df, summary, missing_df, min_missing_share=0.5)
 ...
 # ---------- /quality-flags-from-csv: возвращает только флаги качества ----------
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict[str, dict[str, bool]]:
    """
    Эндпоинт принимает CSV-файл и возвращает словарь с флагами качества,
    включая кастомные флаги из HW03:
    ...
    # Обрезаем пробелы в названиях колонок (на всякий случай)
    ...
    latency_ms = (perf_counter() - start) * 1000.0
    print(f"[quality-flags-from-csv] filename={file.filename!r} latency_ms={latency_ms:.1f} ms")
