...
def report(
    ...
    min_missing_share: float = typer.Option(0.5, help="Порог доли пропусков для выделения проблемных колонок."),
) -> None:
    """
    ...
    # 2. Качество в целом
    quality_flags = compute_quality_flags(df, summary, missing_df, min_missing_share)
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть дубликаты в user_id: **{quality_flags['has_suspicious_id_duplicates']}**\n\n")

        f.write("## Колонки\n\n")
        f.write("См. файл `summary.csv`.\n\n")

        f.write(f"- Порог пропусков для флага 'слишком много пропусков': **{min_missing_share:.2%}**\n")
